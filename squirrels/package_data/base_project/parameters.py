...
def main(proj: Dict[str, Any], *p_args, **kwargs) -> None:
    ...
    query = """
        SELECT '000' as ID, 0 as min_value, max(-Amount) as max_value, 10 as increment \
        FROM transactions WHERE Category <> 'Income'
    """
    max_amount_ds = sr.NumberDataSource(query, "ID", "min_value", "max_value", increment_col="increment", default_value_col="max_value")
