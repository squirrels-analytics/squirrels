...
def user() -> UserBase:
    return UserBase._FromDict({"username": "user1", "organization": "org1"})


class TestMultiSelectParameterConfig:
    def test_with_selection1(
        self, user: UserBase, ms_param_basic: p.MultiSelectParameter, ms_config_basic: pc.MultiSelectParameterConfig
    ):
    ...
    def test_with_selection3(
        self, user: UserBase, ms_param_basic: p.MultiSelectParameter, ms_config_with_ms_parent: pc.MultiSelectParameterConfig
    ):
    ...
    def test_with_selection4(
        self, user: UserBase, ss_param_with_ms_parent: p.SingleSelectParameter, ms_config_with_ss_parent: pc.MultiSelectParameterConfig
    ):
    ...
    def test_invalid_with_selection(self, user: UserBase, ms_config_basic: pc.MultiSelectParameterConfig):
...
class TestSingleSelectParameterConfig:
    def test_with_selection1(
        self, user: UserBase, ms_param_basic: p.MultiSelectParameter, ss_param_with_ms_parent: p.SingleSelectParameter,
    ...
    def test_with_selection3(
        self, user: UserBase, ss_param_with_ms_parent: p.SingleSelectParameter, ss_config_with_ss_parent: pc.SingleSelectParameterConfig
    ):
    ...
    def test_invalid_with_selection(
        self, user: UserBase, ms_param_basic: p.MultiSelectParameter, ss_config_with_ms_parent: pc.SingleSelectParameterConfig
    ):
        with pytest.raises(u.InvalidInputError):
...
class TestDateParameterConfig:
    ...
    def test_invalid_with_selection(self, user: UserBase, date_config_with_parent: pc.DateParameterConfig):
        with pytest.raises(u.InvalidInputError):
...
class TestNumRangeParameterConfig:
    def test_with_selection1(self, user: UserBase, num_range_config: pc.NumRangeParameterConfig):
        curr_option = num_range_config.all_options[1]
        expected = p.NumRangeParameter(num_range_config, curr_option, Decimal("10.9"), Decimal("12.1"))
        param = num_range_config.with_selection("10.9, 12.1", user, None)
        assert param == expected

    def test_invalid_with_selection(self, user: UserBase, num_range_config: pc.NumRangeParameterConfig):
        with pytest.raises(u.InvalidInputError):
