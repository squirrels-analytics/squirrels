...
from squirrels._manifest import ManifestConfig
...
def test_seeds_run_query(simple_manifest_config):
    ...
    seeds = Seeds(
        ...
        _manifest_cfg=simple_manifest_config
    )
...
def test_seeds_get_dataframes(simple_manifest_config):
    seeds_dict = {
        "test": Seed(config=SeedConfig(), df=pl.LazyFrame({"id": [1]}))
    }
    seeds = Seeds(_data=seeds_dict, _manifest_cfg=simple_manifest_config)
...
def test_seeds_io_load_files(simple_manifest_config: ManifestConfig, setup_seed_files: Path):
    logger = Logger("test")
    seeds = SeedsIO.load_files(logger, str(setup_seed_files), simple_manifest_config)
...
def test_seeds_io_load_files_without_config(simple_manifest_config: ManifestConfig, temp_seed_dir: Path, sample_csv_content: str):
    # Test loading CSV without a config file
    csv_path = temp_seed_dir / "test_seed.csv"
    csv_path.write_text(sample_csv_content)
    
    logger = Logger("test")
    seeds = SeedsIO.load_files(logger, str(temp_seed_dir.parent), simple_manifest_config)
