...
from squirrels import _data_sources as d, _parameter_options as po, _parameter_sets as ps, _parameters as p, _parameter_configs as pc
...
def user() -> AbstractUser:
    return create_test_user(organization="org3")
...
@pytest.fixture(scope="module")
def parameter_set0(ss_config_with_ms_parent: pc.SingleSelectParameterConfig) -> ps.ParameterSet:
    data = OrderedDict()
...
@pytest.fixture(scope="module")
def parameter_set1(ms_config_basic: pc.MultiSelectParameterConfig, ss_config_with_ms_parent: pc.SingleSelectParameterConfig) -> ps.ParameterSet:
    data = OrderedDict()
...
def param_configs_set2(
    ...
    ms_ds_param = pc.DataSourceParameterConfig(
        pc.MultiSelectParameterConfig, ms_config_basic.name, ms_config_basic.label, datasource, 
    ...
    date_ds_param = pc.DataSourceParameterConfig(
        pc.DateParameterConfig, date_config_with_parent.name, date_config_with_parent.label, data_datasource, 
...
def test_invalid_overlapping_parent_options():
    ...
    number_options = [
        po.NumberParameterOption(0, 10, parent_option_ids="ss0"),
        po.NumberParameterOption(0, 20, parent_option_ids="ss0")
    ]
    configs_set.add(pc.NumberParameterConfig("child_number", "My Number", number_options, parent_name="parent_ss"))
    with pytest.raises(u.ConfigurationError):
        configs_set._post_process_params({})
...
def test_invalid_overlapping_parent_options_within_user_group():
    ...
    configs_set.add(pc.SingleSelectParameterConfig("parent_ss", "My Single Select", select_options))
    number_options = [
        po.NumberParameterOption(0, 10, parent_option_ids="ss0", user_groups="org1"),
        po.NumberParameterOption(0, 20, parent_option_ids="ss0", user_groups="org2"),
        po.NumberParameterOption(0, 30, parent_option_ids="ss1", user_groups="org2")
    ]
    configs_set.add(pc.NumberParameterConfig("child_number", "My Number", number_options, parent_name="parent_ss"))
    try:
        configs_set._post_process_params({})
    ...
    number_options.append(po.NumberParameterOption(0, 40, parent_option_ids="ss1", user_groups="org2"))
    configs_set.add(pc.NumberParameterConfig("child_number", "My Number", number_options, parent_name="parent_ss"))
    with pytest.raises(u.ConfigurationError):
        configs_set._post_process_params({})
...
def test_apply_selections1(
    user: AbstractUser, param_configs_set1: ps.ParameterConfigsSet, parameter_set0: ps.ParameterSet, parameter_set1: ps.ParameterSet
):
...
def test_apply_selections2(
    user: AbstractUser, param_configs_set2: ps.ParameterConfigsSet, parameter_set2: ps.ParameterSet
):
