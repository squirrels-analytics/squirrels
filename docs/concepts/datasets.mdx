...
In addition to your project's widget parameter names, Squirrels defines a small set of `x_*` request fields. These are **not** widget parameters - they control request behavior (validation, output orientation, limits, etc). See the "Available request fields (per endpoint)" section below for more details.
...
- `x_parent_param` (string) - The parent parameter name used for parameter updates
- `x_verify_params` (bool, **deprecated**) - Use the `x-verify-params` header instead

**Headers**

- `x-verify-params: true|false` - Validates that all provided query/body fields are accepted by the endpoint
- `Authorization` / `x-api-key` - Authentication

<Note>

Without `x_parent_param`, the parameters endpoint accepts **at most 1** widget selection (the server will treat that parameter as the parent). Use `x_parent_param` to be explicit, especially if you need to include additional selections for evaluation.
...
- `x_orientation` (string, **deprecated**) - Use the `x-orientation` header instead
- `x_verify_params` (bool, **deprecated**) - Use the `x-verify-params` header instead

**Headers**

- `x-orientation: records|rows|columns` - Controls result orientation
- `x-verify-params: true|false` - Validates that all provided query/body fields are accepted by the endpoint
- `Authorization` / `x-api-key` - Authentication
