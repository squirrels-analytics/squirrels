...
The Virtual Data Lake (VDL) is the central storage system in Squirrels that holds all materialized data models, seeds, and source data. It is built using [DuckLake], a data lake format that provides a lakehouse architecture with time travel capabilities.
...
-- Reference a source loaded to VDL
...
-- Query without specifying the schema
