...
<Expandable title="Seed configuration fields" defaultOpen>
  ...
  <ResponseField name="columns" type="list" default="[]">
    A list of column configurations. Each column can specify:
    - `name` (required): The column name as it appears in the CSV header
    - `type`: The data type of the column. 
      - If `cast_column_types` is `true`, this field must be set to a [supported type](#supported-types). If the column type is not recognized, an error is raised.
    - `description`: A description of the column
    - `category`: The column category, either `dimension`, `measure`, or `misc`
  </ResponseField>
</Expandable>
...
```sql
-- models/federates/enriched_transactions.sql
SELECT 
    t.transaction_id,
    t.amount,
    c.category
...
```python
# models/federates/enriched_transactions.py
...
| `decimal(my_precision, my_scale)` | `pl.Decimal(my_precision, my_scale)` |
...
- [Project structure](/project/structure) - Overview of files in a Squirrels project
- [Environment variables](/project/env-vars#seeds) - Seed-related environment variables
- [Build models](/project/models/builds) - For larger datasets that need to be materialized
- [Sources](/project/models/sources) - For connecting to existing database tables
